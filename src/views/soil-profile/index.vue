<template>
  <div ref="rootRef" class="soil-profile-page geo-template-page geo-page theme-light" :class="'layout-' + layoutMode">
    ...
    <main class="workspace" v-bind="workspaceAttrs">
      <aside class="side-panel left-panel" v-bind="leftPanelAttrs">
      ...
      <button v-if="leftCollapsed" type="button" class="panel-entry-btn entry-left" v-bind="leftEntryAttrs">
        ›
      </button>
    </main>
...
const analysisPanelCollapsed =
  ref(false)

/*
 * 本页面只有一个模板侧栏。
 * 右上角“宏观剖面理化分析”属于主场景业务浮层，
 * 不作为公共模板的右侧面板。
 */
const {
  rootRef,
  ...
  onLayoutChange(state) {
    /*
     * 面板拖拽和浏览器连续缩放期间，
     * 不反复重建 Three.js 与 ECharts 画布。
     */
    if (state.resizing) {
      return
    }

    scheduleTemplateResize(90)
  },
...
function toggleLeftPanel() {
  toggleTemplatePanel('left')
}

function toggleAnalysisPanel() {
  analysisPanelCollapsed.value =
    !analysisPanelCollapsed.value

  /*
   * 分析面板使用 v-show。
   * 重新显示 ECharts 时必须等 DOM 可见后再 resize。
   */
  nextTick(() => {
    scheduleTemplateResize(0)
  })
...
function isTemplateLayoutResizing() {
  return (
...
function scheduleTemplateResize(
  ...
  if (
    ...
    pendingSceneWidth =
      Math.max(
        1,
        Math.round(observedWidth)
      )
  }

  if (
    typeof observedHeight === 'number' &&
    Number.isFinite(observedHeight)
  ) {
    pendingSceneHeight =
      Math.max(
        1,
        Math.round(observedHeight)
      )
  }
  ...
  /*
   * 拖拽过程中只让 DOM 与 canvas CSS 尺寸跟随。
   * 松手后由 Hook 再触发一次最终校准。
   */
  if (isTemplateLayoutResizing()) {
    return
  }
        ...
        requestAnimationFrame(() => {
          ...
          sceneResizeSettleFrame =
            requestAnimationFrame(() => {
              ...
              resizeScene(
                pendingSceneWidth || undefined,
                pendingSceneHeight || undefined
              )

              /*
               * ECharts 位于分析浮层中。
               * 不在 pointermove 阶段高频重建画布。
               */
              mapChart?.resize()
            })
        })
...
function resizeScene(
  ...
  const width = Math.max(
    observedWidth ??
    Math.round(
      sceneContainerRef.value.clientWidth
    ),
    1
  )
  ...
  const height = Math.max(
    observedHeight ??
    Math.round(
      sceneContainerRef.value.clientHeight
    ),
    1
  )

  /*
   * renderer.setSize 会重新分配 WebGL drawing buffer。
   * 尺寸未变化时绝不重复调用。
   */
  if (
...
function bindMicroResizeObserver() {
  const container = microContainerRef.value
  if (!container) return
...
function initMicroScene() {
  ...
  // 弹窗使用 v-if，关闭后旧 canvas 会从 DOM 中移除。
  // 再次打开时复用已初始化场景，并把 canvas 重新挂到新容器。
  if (microScene && microRenderer && microCamera) {
    if (microRenderer.domElement.parentElement !== container) {
...
onMounted(async () => {
  ...
  /*
   * 3D 动画不再等待 GeoJSON / ECharts 异步加载。
   */
...
/* ===================== v14：公共面板 Hook + 微观场景 resize 修复 =====================
   左侧面板宽度、断点、平板触控拖拽与收起展开，
